# Born2BeRoot 🛡️
    ...
    ## 📝 Project Overview
      Born2BeRoot is a System Administration project that involves setting up a **Virtual
      Machine (VM)** specifically configured to adhere to strict security policies. The goal
      to implement a secure server environment using **Debian (or CentOS)**, managing
      partitions, users, permissions, and automated monitoring.
    ...
    - **Authentication:** SSH Key-based authentication preferred; Password authentication
      secured.
    ...
    A custom Bash script (`src/scripts/monitoring.sh`) runs every 10 minutes (via Cron) to
    broadcast system health to all logged-in terminals.
  ...
  ram_percent=$(free | awk '$1 == "Mem:" {printf("%.2f"), $3/$2*100}')
    ...
    ## 📸 Visual Proofs
     ...
     | **Monitoring Output** | ![Wall](img/monitoring_broadcast.png) |
